...
//		System.out.println("Starting game!");
		...
		while(board.gameNotOver() && !gameOver){
			if(myTurn){
				...
				if(moves.isEmpty()){/*Error? Not needed.*/}
...
//		System.out.print(", time="+time);
	...
	KalahPlayer getPlayer(Board board){
		...
		try{
			...
			else return (KalahPlayer) Class.forName("AI."+settings.getString("AI-name"))
										.getConstructor(Board.class).newInstance(board);
		}
		//Lots of potentials reflection errors
		//(but these don't happen since we load class names directly in EditSettingsWindow)
	...
	boolean parseServerMessage(String... args){
		...
		if(args[0].equals("WELCOME")){
			...
			//Set up the game using the INFO
			...
			timer.cancelTimer();//The move timed out
			if(myTurn) result = GameResult.TIME;
		}
		else if(args[0].equals("ILLEGAL")){
			//The server thinks I am cheating (is the server wrong?)
	...
	@Override public void receiveMessage(String message) {
		...
		if(args[0].matches("^\\d+$")){
			...
			//Unable to parse message from client
